...
/**
 * Compute which co-located data file, if any, each asset actually has.
 *
 * The studio cannot stat the filesystem, so without this it discovers the
 * answer empirically — importing `<Name>.data.js` and falling through to
 * `<Name>.data.json` when the first 404s. That guess is wrong for most assets
 * (a `.data.js` is the rare case), so a normal page load litters the console
 * with 404s for files that were never expected to exist, and repeats them on
 * every auto-refresh tick.
 *
 * The server already knows: `loadAssetData` reads each asset's folder and
 * applies core's `.data.js`-over-`.data.json` precedence (FR22). Shipping the
 * answer in the virtual module turns a blind probe into a single, correct
 * request — and no request at all for an asset with no data file.
 *
 * Serialized as `Array<[assetPath, dataPath]>`, omitting assets with no data
 * file. Never throws: on failure it yields `[]`, and the studio falls back to
 * the old probing path, which still works.
 *
 * @param {object | null} project
 * @param {import('@lerret/core').FilesystemAccess} backend
 * @returns {Promise<Array<[string, string]>>}
 */
async function computeAssetDataEntries(project, backend) {
  if (!project) return [];
  try {
    const map = await loadAssetData(collectAssets(project), backend);
    const entries = [];
    for (const [assetPath, record] of map.entries()) {
      if (record && record.source !== 'absent' && typeof record.dataPath === 'string') {
        entries.push([assetPath, record.dataPath]);
      }
    }
    return entries;
  } catch (err) {
    console.error(
      '[lerret] asset-data discovery failed:',
 ...
 */
function buildVirtualModuleSource({ project, assetBaseUrl, projectRoot, lerretDir, epoch, cascadeEntries, assetConfigEntries, assetDataEntries, overrides }) {
  return [
    ...
    // assetDataEntries: Array<[assetPath, dataPath]> — the co-located data
    // file each asset actually has, per core's `.data.js` > `.data.json`
    // precedence. Assets with no data file are omitted. The studio fetches
    // exactly this path instead of probing, so a missing data file costs no
    // request (and no console 404).
    `export const assetDataEntries = ${JSON.stringify(assetDataEntries || [])};`,
    ...
    `export default { project, assetBaseUrl, projectRoot, lerretDir, epoch, cascadeEntries, assetConfigEntries, assetDataEntries, overrides, mode };`,
    '',
  ].join('\n');
}
  ...
  return {
    ...
    load(id) {
      ...
      return buildVirtualModuleSource({
        ...
        assetDataEntries: currentAssetDataEntries,
    ...
    async configureServer(server) {
      ...
      async function rescanCurrentProject() {
        ...
        try {
          ...
          currentAssetDataEntries = await computeAssetDataEntries(currentProject, backend);
        } catch (err) {
