...
**Because everything is React, every LLM already knows how to edit it.** No plugin, MCP, or design-tool SDK *required* — just files an AI was trained on millions of times over. (There is an optional [Claude Code plugin](/ai-editing/#the-claude-code-plugin) that turns this into a single `/lerret` command.)
