...
|---|---|---|
| **Local CLI** | A Node process running `@lerret/cli dev` against a folder on disk. Native Vite HMR. | Your daily dev loop. |
...
<PM cmd="npx @lerret/cli@latest dev" />
...
**Works in any browser** — the CLI mode does not depend on the File System Access API. Safari and Firefox users can run `@lerret/cli dev` even though the hosted studio is Chromium-only.
...
See the [CLI Reference](/cli/#lerretcli-dev) for the full flag list.

## Hosted mode

The studio published as a **static SPA** served from `app.lerret.belikely.com` (Belikely-hosted), or any static host you control (see [Self-host](#self-host) below).
...
Don't have a project handy? The connect screen also offers **"Try a demo — no folder needed"**, which loads a sample showcase project entirely in the browser — useful for a first look before scaffolding anything.
...
- Running `npx @lerret/cli@latest dev` locally instead.
...
| Run a CI export pipeline (no GUI) | **CLI**, via `@lerret/cli export` |
